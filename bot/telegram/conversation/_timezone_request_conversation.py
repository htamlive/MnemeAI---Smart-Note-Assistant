...
class TimezoneRequestConversation(CommandConversation):
    def __init__(self, TIMEZONE_REQ: int, client: TelegramClient, debug: bool = True) -> None:
        ...
        self._states = [MessageHandler(filters.LOCATION, self.receive_user_location)]

    async def start_conversation(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        await update.message.reply_text(
            "Please press the button to share your location.", 
    ...
    async def receive_user_location(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
